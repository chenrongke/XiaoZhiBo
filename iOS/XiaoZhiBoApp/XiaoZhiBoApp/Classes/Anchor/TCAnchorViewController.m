...
- (void)dealloc {
    ...
    [_logicView.vMusicPanel resetAudioSetting];
}
...
- (void)viewDidLoad {
    ...
    [_logicView.vMusicPanel setAudioEffectManager:[_liveRoom getAudioEffectManager]];
    _logicView.vMusicPanel.delegate = self;
...
-(void)onEffectViewHidden:(BOOL)isHidden {
    [_logicView setButtonHidden:!isHidden];
}
...
//    [self.liveRoom switchCamera];
    [self.liveRoom stopLocalPreview];
    //跳转录屏直播
    ScreenPushViewController *vc = [[ScreenPushViewController alloc] init];
